...
**Status:** data, chunking, embeddings, and search; no scoring yet. 10 NBA history Wikipedia articles (~97,000 words) in `data/documents/` and 50 evaluation questions in `data/eval/questions.jsonl`; `rag/` turns those into `Document` and `Chunk` objects, embeds them locally with `all-MiniLM-L6-v2`, and searches them exactly with NumPy. Rebuild the corpus with `python3 scripts/fetch_corpus.py`, check the questions with `python3 scripts/validate_eval.py`, run tests with `python3 -m unittest discover tests`. Everything except embedding is standard library only; embedding needs `pip install -r requirements.txt` (roughly 1–2 GB, since it pulls in PyTorch).
